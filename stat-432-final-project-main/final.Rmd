...
The second paper, titled "Car Price Prediction Using Machine Learning Techniques" explores various machine learning methods to predict used car prices. The study focuses on utilizing structured data to create models capable of determining car prices based on features like make, model, year of manufacture, and mileage. While the authors of paper did not use the dataset we did, and instead scraped the web for their data, their methodology of data processing was of interest. Attributes like car age, manufacturer, mileage, and specifically, features available to the car were of great importance. This is part of the reason why we decided to split up and use the engine variable to gain more information about the car features. 
The authors analyzed correlations between features and identified key insights, such as the way that year of manufacture impacts car price slightly, allowing us to make similar decisions about what variables we chose to work with. The dataset amassed for this study was divided into two subsets: training (70%) and testing (30%). They constructed models using Random Forest and Support Vector Machine (SVM), and a Neural Network. Eventually, they decided to use an ensemble method, that while computationally more laborious, was a lot more accurate.
...
- ``https://www.researchgate.net/publication/379443793_CAR_PRICE_PREDICTION_USING_MACHINE_LEARNING_TECHNIQUES`` by Yavuz selim, Balcioglu
Bulent Sezen
...
Within our dataset, we use the Horsepower, Engine_Size, and Cylinders variable, which were all extracted from the original CSV's engine variable. To deal with the null/unavailable values in the dataset, the following strategies were chosen:

We choose the mean for engine size and horsepower because they are both numeric features with a wide range of values, making the mean a reasonable representative measure for the dataset. We use the mode for cylinders because while cylinder count is numeric, the values are discrete and often limited to a small set of options. This makes the mode an intuitive choice. For clean_title, we choose to replace with "No", since we want to take a cautious approach and assume if there is no information about a clean_title, the car cannot be marketed as such. For fuel_type and transmission, we remove the rows with null value since these are variables that impact the data, cannot be replaced with an estimate, and there are relatively few occurences, causing us to lose only 49 rows of data. For int_color, we set null values to Unkown, so as to avoid having null values in the data, and take a cautious approach. We also removed outliers from the data by fitting a saturated linear model and using Cook's Distance as our metric, to obtain our final cleaned data.
...
\textbf{Hierarchical Clustering}: We now implement Hierarchical Clustering
```{r, echo=FALSE, fig.align='center', out.width='80%'}
knitr::include_graphics("qqqq.png")
```
Using this we can also see that $k = 7$ is the optimal number of clusters to distinguish the data.

\textbf{Spectral Clustering}: Let us consider spectral clustering on the most relevant numerical variables available to us, to try and find some thresholds with which to cluster the data. We decide to use the Eigengap heuristic to try and determine the ideal number of clusters. 
```{r, echo=FALSE, fig.align='center', out.width='80%'}
...
```{r, echo=FALSE, fig.align='center', out.width='80%'}
knitr::include_graphics("something.png")
```
\begin{table}[h!]
\centering
\begin{tabular}{|l|c|c|}
...
\textbf{XGBoost}: The goal of this analysis is to predict our vehicle prices using an XGBoost regression model. Categorical features, such as brand, model, fuel_type, transmission, ext_col, int_col, accident, and clean_title, were all explicitly converted into numerical format using factor encoding. The dataset was split into training (75%) and testing (25%) subsets to evaluate the model's performance on unseen data.
...
\textbf{(Single) Regression Tree}: We can also try to use a regression tree, which is useful for its interpretability. While the prediciton of tree models in this simple context tends to be not ideal, we would also like to first display this core component of random forests and boosting before turning our attention to these methods. So, first, we split our data into testing and training. Then, we fit our tree model. The set of variables we use were chosen because they were easily branchable, and were interpretable in the tree format. Things like the brand and model were ommitted, because they obscured the relationship of price to most variables in favor of granular results. 
```{r, echo=FALSE, fig.align='center', out.width='80%'}
knitr::include_graphics("q.png")
knitr::include_graphics("qq.png")
```
It has a Train RMSE of 50662.54, a Test RMSE of 62926.23, a Train Rsquared of 0.4639864 and a Test Rsquared 0.4132156. Very clearly, the regression tree was not able to capture the complexity of our data, so, we should delve further into techniques to try and rectify this.

\textbf{Support Vector Regression}: For the implementation of the SVR model, we used a grid search strategy for hyperparameter tuning, exploring combination of cost parameter $C \in {0.1, 1, 10}$ and sigma $\in {0.01, 0.1, 1}$ using 5-fold cross validation to prevent overfitting as much as possible. We selected the Radial Basis function (RBF) kernel as it also is able to capture any non-linear trends (although this might not be a major problem for the data we are working with). For feature engineering we perform encoding on categorical variables and removed features such as model names and scaled numerical features. We use RMSE and $R^2$ to evaluate the model performance.  
...
```
Upon analyzing the results of multiple runs from the SVR code, the best chosen sigma value was 0.1 and the best chosen C value was 1. These produced a testing RMSE of 60512.02 with an $R^2$ of 0.2448565 and a MAE value of 26055.05. From the plot we can also see that SVR puts emphasis on a mix of categorical and numerical data. We can see that overall the SVR model has major room for improvement which can be explained by high dimensionality created by the encoding of categorical variables. The model is likely to have also taken a toll when it comes to outliers as SVR can be rather sensitive to this. While we did remove influential points with high cooks distance in our initial processing, some outliers still remain. We also use a rather small grid space due to run-time constrains.

# Open Ended Question

For the open ended question we implement a Random Forest model. We built a Random Forest model that predicts original car prices by first adjusting the used car prices in our dataset for inflation and age-based depreciation. We engineered features that capture usage patterns (miles per year) and vehicle characteristics (horsepower/engine ratio), then preprocessed the data by converting categorical variables like brand and transmission into numeric form. The model learns from these features to essentially "work backward" from used prices to estimate what cars might have cost when new.
```{r, echo=FALSE, cache = TRUE, message=FALSE, warning=FALSE}
library(tidyverse)
library(ranger)
library(recipes)
cars_data <- read.csv("final_data.csv")
current_year <- 2024
cars_data <- cars_data %>%
  mutate(
    age = current_year - model_year,
    adjusted_price = Price * (1.03^age)
  )
...
model_data <- cars_data %>%
  mutate(
    ...
    hp_ratio = Horsepower / Engine_Size
  ) %>%
  filter(age > 0, Mileage > 0)  
model_data <- model_data %>%
  mutate(across(c(brand, fuel_type, transmission), as.factor))

recipe_obj <- recipe(adjusted_price ~ brand + model_year + fuel_type + 
                    transmission + Horsepower + Engine_Size + 
                    Cylinders + miles_per_year + hp_ratio, 
                    data = model_data) %>%
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_normalize(all_numeric(), -all_outcomes()) %>%
  step_zv(all_predictors())

prep_obj <- prep(recipe_obj, training = model_data)
processed_data <- bake(prep_obj, new_data = model_data)
...
rf_model <- ranger(
  ...
  data = processed_data,
  num.trees = 500,
  mtry = floor(sqrt(ncol(processed_data) - 1)),
  min.node.size = 5,
  importance = 'impurity'
)

predict_original_price <- function(car_data, model, recipe_obj) {
  car_processed <- bake(recipe_obj, new_data = car_data)
  predicted_price <- predict(model, data = car_processed)$predictions
  
  return(predicted_price)
}

sample_cars <- cars_data %>%
  filter(
    brand == "BMW" & model_year >= 2020 |
    brand == "Toyota" & model_year >= 2020 |
    brand == "Porsche" & model_year >= 2020
  ) %>%
  head(3)

prediction_data <- sample_cars %>%
  mutate(
    age = current_year - model_year,
    miles_per_year = Mileage / age,
    price_per_mile = Price / Mileage,
...
original_prices <- predict_original_price(prediction_data, rf_model, prep_obj)

results <- cbind(
  prediction_data %>% select(brand, model, model_year, Price, Mileage),
  Estimated_Original_Price = original_prices
) %>%
  mutate(
    Price = round(Price, 0),
    Estimated_Original_Price = round(Estimated_Original_Price, 0)
  )

print("Selected Cars and Their Estimated Original Prices:")
print(results)
```
Comparing the original release prices from the internet search we find that the 2021 Toyota	Supra 3.0 Premium had a MSRP of 54590 dollars, which is lower than the model 2020 BMW 440 Gran Coupe 440i xDrive had a MSRP of 53150 dollars which is also lower than the estimation of 61235 dollars and the 2020 BMW i3 120Ah w/Range Extender had a MSRP of 48300. The model's estimates differ from actual MSRPs because it overly compensates for luxury brand depreciation and lacks crucial information about specific trim levels and optional features that affect original prices. Additionally, our simple inflation adjustment doesn't capture the complex pricing dynamics of the auto industry, especially during 2020-2021 when market conditions were unusual due to COVID-19 and supply chain disruptions. The model essentially tries to infer too much from limited data, leading to overestimation particularly for luxury vehicles.
